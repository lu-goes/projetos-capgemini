...
public class Matrizes {
		...
		*/
		...
		// lado externo para associar aluno a nota
		for (int i = 0; i < 3; i++) {
			System.out.println("Aluno : " + alunos[i]);
			// lado interno para informar as notas de cada aluno
			somaNotas = 0;
			mediaAluno = 0;
			for (int j = 0; j < 4; j++) {
				notas[i][j] = Float.parseFloat(JOptionPane.showInputDialog("Informe a nota no " + (j+1) + " do aluno " + alunos[i]));
